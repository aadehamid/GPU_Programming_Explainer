...
__global__ void scalarMult2DArray(float* a, float* b, float* result, int width, int height){
...
int main(){
	...
	cudaMalloc(&cuda_b, WIDTH * HEIGHT * sizeof(float));
	...
	cudaMemcpy(cuda_b, b.data(), WIDTH * HEIGHT * sizeof(float), cudaMemcpyHostToDevice);
	// Launch the kernel
	scalarMult2DArray<<<blocksPerGrid, threadsPerBlock>>>(cuda_a, cuda_b, cuda_result, WIDTH, HEIGHT);
